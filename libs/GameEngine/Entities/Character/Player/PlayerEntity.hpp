...
public:
    PlayerEntity();
    PlayerEntity(const std::string& path, float x, float y, int playerNumber, int uid);
    ~PlayerEntity();
    ...
    void clock_timer(sf::Vector2f pos, ServerData &serverData);
    void setUid(int uid);
    int getUid();
    int getMissileSize();
    virtual int getId() override;
    std::vector<std::unique_ptr<MissileEntity>>& getMissiles();
protected:
