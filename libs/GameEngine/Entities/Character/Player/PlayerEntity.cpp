...
PlayerEntity::PlayerEntity(const std::string& path, float x, float y, int playerNumber) : CharacterEntity(path, x, y, playerNumber)
{
}
...
void PlayerEntity::Update(sf::RenderWindow& window, ServerData &serverData)
{
    ...
    if (sf::Keyboard::isKeyPressed(sf::Keyboard::Left)) {
        if (!(sprite.getPosition().x <= 20)) {
            ...
            + std::to_string(this->getUid()) + " entityType:Player" + " positionX:" 
    ...
    if (sf::Keyboard::isKeyPressed(sf::Keyboard::Space)) {
        // commands cmd;
        // cmd.sendCommand("1 connect", serverData);
...
void PlayerEntity::clock_timer(sf::Vector2f pos)
{
    ...
    if (seconds > 10.5) {
        addMissile(std::make_unique<MissileEntity>(pos.x, pos.y, MissileEntity::DIRECTION::EAST));
        clock.restart();
    }
}
